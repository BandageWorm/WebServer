...
public class Word2VEC {
    ...
    public static void main(String[] args) throws IOException {
        ...
        vec.setTopNSize(5);
        System.out.print(vec.distance("城市"));
    }
    ...
    public void modifyJavaModel(String path) throws IOException {
        DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(path)));
        DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(FilePath.get("Text\\new_w2v"))));
        words = dis.readInt();
        size = dis.readInt();
        dos.writeInt(words);
        dos.writeInt(size);
    ...
    public void loadNewJavaModel(String path) throws IOException {
        try (DataInputStream dis = new DataInputStream(new BufferedInputStream(new FileInputStream(path)))) {
            words = dis.readInt();
            size = dis.readInt();
    ...
    public double distance2Words(String word1, String word2) {
        float[] center1 = wordMap.get(word1);
        float[] center2 = wordMap.get(word2);
        if (center1 == null || center2 == null) return 0.0;
        double dics = 0;
    ...
    public double cosSimilarity(String word1, String word2) {
        float[] center1 = wordMap.get(word1);
        float[] center2 = wordMap.get(word2);
        if (center1 == null || center2 == null) return 0.0;
        double deno = 0;
        ...
        double len2 = 0;
        for (int i = 0; i < size; i++) {
            deno += center1[i] * center2[i];
            len1 += center1[i] * center1[i];
            len2 += center2[i] * center2[i];
        }
        len1 = Math.sqrt(len1);
        len2 = Math.sqrt(len2);
        return deno / (len1 * len2);
    }
